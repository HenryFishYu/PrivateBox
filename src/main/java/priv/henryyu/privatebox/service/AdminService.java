	...
	*/
	public ResponseMessage<List<InvitationCode>> generateInvitationCodes(String amount) {
		...
		if (empty.equals(amount) || !StringUtils.isNumber(amount)) {
			...
			responseMessage.setMessage(messageSource.getMessage("illegalInputNumber", null, locale));
	...
	/**
	* 查询所有本人邀请码
	* Query all InvitationCodes generated by self
	* @param 邀请码查询表单 
	* InvitationCodeQueryForm
	* @return DataGrid
	* @throws Exception
	*/
	public DataGrid<InvitationCode> queryInvitationCodes(InvitationCodeQueryForm invitationCodeQueryForm) {
		...
		if (invitationCodeQueryForm.getSort() == null || invitationCodeQueryForm.getOrder() == null) {
			...
			Sort sort = new Sort(Direction.fromString(invitationCodeQueryForm.getOrder()),
					invitationCodeQueryForm.getSort());
			pageable = new PageRequest(invitationCodeQueryForm.getPage() - 1, invitationCodeQueryForm.getRows(), sort);
		}
		Page<InvitationCode> page = invitationCodeRepository.findByCreateUsernameAndCodeLike(getUser().getUsername(),
				"%" + invitationCodeQueryForm.getCode() + "%", pageable);
